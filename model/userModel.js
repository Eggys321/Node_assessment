...
userSchema.methods.getVerifyEmailToken = function(){
  const verifyEmailTokens = crypto.randomBytes(20).toString('hex');
  this.verifyEmailToken = crypto.createHash("sha256")
  .update(verifyEmailTokens)
  .digest('hex');
  this.verifyEmailExpire = Date.now() + 10 * (60 * 1000);
  return verifyEmailTokens
}
